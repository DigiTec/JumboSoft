...
(function() {
  function _scrollIntoFocus(uiElement) {
    this.uiPanel.scrollLeft =
      uiElement.uiRoot.offsetLeft - this.uiLeft.offsetWidth;
  ...
  function _finalizeScroll(evt) {
    var uiOffset = Math.clamp(
      Math.round(
        this.uiPanel.scrollLeft / this.uiElements[0].uiRoot.offsetWidth
      ),
      0,
      this.uiElements.length - 1
    );
  ...
  Object.defineProperties(AchievementUI.prototype, {
    init: {
      value: function init() {
        ...
        this.achievements.subObjects.forEach(function _setupAchievementUI(
          achievement
        ) {
          var uiElement = {
          ...
          uiElement.uiRoot.addEventListener(
            "click",
            _clickUIElement.bind({ ui: this, uiElement: uiElement })
          );
        ...
        if (this.achievements.achievementsChanged) {
          ...
          for (var i = 0; i < achievementCount; i++) {
            ...
            if (achievement != uiElement.achievement) {
              throw new Error(
                "The achievements lists and UI lists are out of sync :-("
              );
            }
