...
Object.defineProperties(JumbosoftGame.prototype, {
    triggerNames: {
        value: ["totalStocks", "handMadeStocks", "stockTicker", "clicks", "gameYears"]
    },
    getTriggerValue: {
    ...
    save: {
        value: function save(rootObject) {
            if (rootObject.hasOwnProperty("game")) {
                throw new Error("Property game is already present during save. Duplicate save.");
            }
    ...
    loadGame: {
        value: function load() {
            if (window.localStorage["saveGame"]) {
                var rootObject = JSON.parse(window.atob(window.localStorage["saveGame"]));
