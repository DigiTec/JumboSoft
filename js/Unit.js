...
function Unit(unitList, name, trigger, description, initialPrice, scaleFactor, stockPower, upgrades) {
...
Object.defineProperties(Unit.prototype, {
    computePurchasePrice: {
        value: function computePurchasePrice() {
            return Math.floor(Math.geometricSeriesValue(this.initialPrice, this.scaleFactor, this.owned));
        }
    },
    buy: {
        value: function buy() {
    ...
    computeTotalUpgrades: {
        value: function computeTotalUpgrades() {
            if (this.upgradesChanged) {
                ...
                this.totalUpgrades = this.subObjects.reduce(function _reduceUnitUpgrades(prev, curr) {
                    if (curr.unlocked) {
                        return prev + 1;
                    }
                    else {
    ...
    computeStockValue: {
        value: function computeStockValue() {
            var stockValue = this.subObjects.reduce(function _reduceUpgrades(prev, curr) {
                if (curr.unlocked) {
